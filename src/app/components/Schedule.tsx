...
function ScheduleTabbed() {
  ...
  return (
    <TabGroup
      ...
      <TabPanels>
        {schedule.map(day => (
          <TabPanel key={day.dateTime} className="ui-not-focus-visible:outline-none">
            <TimeSlots day={day} />
          </TabPanel>
        ))}
      </TabPanels>
...
function DaySummary({ day }: { day: Day }) {
  return (
    <>
      <h3 className="text-2xl font-semibold tracking-tight text-tertiary">
        <time dateTime={day.dateTime}>{day.date}</time>
      </h3>
      <p className="mt-1.5 text-base tracking-tight text-tertiary">{day.summary}</p>
    </>
  )
}
...
function TimeSlots({ day, className }: { day: Day; className?: string }) {
  return (
    <ol
      ...
      className={clsx(
        className,
        "space-y-8 bg-white/60 px-10 py-14 text-center shadow-xl shadow-tertiary/5 backdrop-blur",
      )}>
      {day.timeSlots.map((timeSlot, timeSlotIndex) => (
        <li
          ...
          <h4 className="text-lg font-semibold tracking-tight text-tertiary">{timeSlot.name}</h4>
          {timeSlot.description && <p className="mt-1 tracking-tight text-tertiary">{timeSlot.description}</p>}
    ...
    <section id="schedule" aria-label="Schedule" className="py-20 sm:py-32">
      <Container className="relative z-10">
        <div className="mx-auto max-w-2xl lg:mx-0 lg:max-w-4xl lg:pr-24">
          ...
          <p className="mt-4 font-display text-2xl tracking-tight text-tertiary">
